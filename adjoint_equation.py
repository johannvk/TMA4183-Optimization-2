...
class AdjointEquationSolver():
    ...
    def __init__(self, mesh: fe.Mesh, spatial_function_space: fe.FunctionSpace, T: float, steps:int=50, eps: float=1.0):
    ...
    def solve(self, save_steps=True, save_to_file=False, filename=''):
        ...
        for i in range(self.time_steps):
            ...
            # Should only need to assign 'self.A' once. Not for every time step.
...
def island_init_cond(ndof=64, T=0.05, steps=10, eps=0.1):
    ...
    ae_solver = AdjointEquationSolver(mesh=mesh, spatial_function_space=V, T=T, steps=steps, eps=eps)
